...
/**
 * @Title: SelectCountSqlInterceptor2
    ...
    private void processCountSql(final Object[] queryArgs)
    {
        if (queryArgs[PARAMETER_INDEX] instanceof Map)
        {
            ...
            if (parameter.containsKey(COUNT))
            {
                ...
                BoundSql newBoundSql = new BoundSql(ms.getConfiguration(), getCountSQL(sql), boundSql.getParameterMappings(), boundSql.getParameterObject());


                MappedStatement newMs = copyFromMappedStatement(ms, new SqlSource() {
                    @Override
